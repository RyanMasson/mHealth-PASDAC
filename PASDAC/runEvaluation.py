...
def runEvaluation(features, fType, fDescr, segments, segmentation, labelsSegmentation, SETTINGS):
    ...
    for iFold in range(SETTINGS.FOLDS):
        ...
        if SETTINGS.SAVE == 1:
            if not os.path.exists(SETTINGS.PATH_OUTPUT_FEATURES):
                os.makedirs(SETTINGS.PATH_OUTPUT_FEATURES)
            trainingData.to_csv(SETTINGS.PATH_OUTPUT_FEATURES + "/trainingDataFold" + str(iFold) + ".csv", index = None)
            trainingLabels.to_csv(SETTINGS.PATH_OUTPUT_FEATURES + "/trainingLabelsFold" + str(iFold) + ".csv", index = None)
            testData.to_csv(SETTINGS.PATH_OUTPUT_FEATURES + "/testDataFold" + str(iFold) + ".csv", index = None)
            testLabels.to_csv(SETTINGS.PATH_OUTPUT_FEATURES + "/testLabelsFold" + str(iFold) + ".csv", index = None)
        ...
        offset = max(cvTestSegmentation['End'].as_matrix())
    ...
    if SETTINGS.SAVE == 1:
        cvTestSegmentation.to_csv(SETTINGS.PATH_OUTPUT + "/cvTestSegmentation.csv", index = None)
        scoresTimeseries.to_csv(SETTINGS.PATH_OUTPUT + "/scoresTimeseries.csv", index = None)
        # ground truth labels,  point by point 
        # type: Series
        cvTestLabels.to_csv(SETTINGS.PATH_OUTPUT + "/cvTestLabels.csv", index = None)
        # cvTestLabels starts from 1
        labelsTimeseries.to_csv(SETTINGS.PATH_OUTPUT + "/labelsTimeseries.csv", index = None)
    ...
    if SETTINGS.SAVE == 1:
        finalPredTimeseries.to_csv(SETTINGS.PATH_OUTPUT + "/finalPredTimeseries.csv", index = None)
    ...
    plotROC(cvTestLabels.as_matrix()-1, probas.as_matrix(), SETTINGS.CLASSES)
